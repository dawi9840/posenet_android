...
/*    // Create rotated version for portrait display (source)
  ...
  private fun draw(canvas: Canvas, person: Person, bitmap: Bitmap, startTime: Long) {
    ...
    canvas.drawText("Score: %.2f".format(person.score), (15.0f * widthRatio), (30.0f * heightRatio + bottom), paint)
    canvas.drawText( "Device: %s".format(posenet.device), (15.0f * widthRatio), (50.0f * heightRatio + bottom), paint)
    canvas.drawText("Inference time: %.2f ms".format(posenet.lastInferenceTimeNanos * 1.0f / 1_000_000), (15.0f * widthRatio), (70.0f * heightRatio + bottom), paint)
    canvas.drawText("Pipeline time so far: %.2f ms".format((System.currentTimeMillis() - startTime).toFloat()), (15.0f * widthRatio), (90.0f * heightRatio + bottom), paint)
    */

    //draw txt device info details
    canvas.drawText("Score: %.2f".format(person.score), (5.0f * widthRatio), (190.0f * heightRatio + top), paint)
    canvas.drawText("Device: %s".format(posenet.device), (5.0f * widthRatio), (220.0f * heightRatio + top), paint)
    canvas.drawText("Time: %.2f ms".format(posenet.lastInferenceTimeNanos * 1.0f / 1_000_000), (5.0f * widthRatio), (250.0f * heightRatio + top), paint)
  ...
  /** Process image using Posenet library.   1*/
