...
class Posenet(
...
) : AutoCloseable {         /** Kotlin中任何實現Closeable interface的資源都會被自動關閉*/
  ...
  private fun getInterpreter(): Interpreter {
    ...
    options.setNumThreads(NUM_LITE_THREADS) //Sets the number of threads to be used for ops that support multi-threading.
    when (device) {
      Device.CPU -> { }
      Device.GPU -> {
        gpuDelegate = GpuDelegate()
        options.addDelegate(gpuDelegate) //Adds a Delegate to be applied during interpreter creation.
      }
      Device.NNAPI -> options.setUseNNAPI(true) //Sets whether to use NN API (if available) for op execution.
    }
  ...
  private fun initInputArray(bitmap: Bitmap): ByteBuffer {
    ...
    ) /**  ByteBuffer 分為兩種: direct 與non-direct。Direct buffer 基本上是映射在 java heap 之外的, 而且可以直接讓作業系統存取, 因此其速度是比使用陣列的 non-direct buffer 要來得快的。Non-direct buffer 是透過 allocate() 來建立的, 且其基本上都會有一個 array 作為備份資料的空間, 故這個 array 又稱為 backing array; 而 direct buffer 則是透過allocateDirect()來建立。 */
    inputBuffer.order(ByteOrder.nativeOrder()) //獲取底層平臺的本機位元組順序。
    inputBuffer.rewind()
  ...
  private fun initOutputMap(interpreter: Interpreter): HashMap<Int, Any> {
    val outputMap = HashMap<Int, Any>()     /** HashMap<key, value> */
  ...
  fun estimateSinglePose(bitmap: Bitmap): Person {
    ...
    /*Log.i(
      "osenet",
      String.format(
        "InputArray scaling to [-1,1] took %.2f ms",
    ...
    */
    ...
    enumValues<BodyPart>().forEachIndexed { idx, it ->
      ...
      Log.i("position", "estimateSinglePose: ${keypointList[idx].position}")
